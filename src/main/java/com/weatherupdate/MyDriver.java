...
public class MyDriver extends Configured implements Tool {
    ...
    private void parseArgs(String[] args,Job job ,FileSystem fs) throws Exception {
        ...
        TextInputFormat.addInputPath(job,new Path(this.parseInputDirectoryArg(args)));
...
/*
  ...
  */
  ...
  private String deleteOutputDirectory(FileSystem fs,Path directory)
    {
        try {
    ...
    private String parseInputDirectoryArg(String[] args)
    {
        int index=0;
        ...
        if ( args.length > index)
        {
            ...
            inputDirectory2=arg[1];
            logger.info(" InputDirectory2 " + inputDirectory2);
        }
        else
...
//setting the InputPath 
     ...
     MultipleInputs.addInputPath(job, new Path(this.parseInputDirectoryarg(arg),TextInputFormat.class, MyMapper.class);
     MultipleInputs.addInputPath(job, new Path(args[1]),TextInputFormat.class, MyMapper.class);
      ...
      job.submit();
        ...
        {
            logger.error("An error has occured while trying to create
the hadoop object");
        }
